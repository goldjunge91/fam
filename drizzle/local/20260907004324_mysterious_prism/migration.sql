ALTER TABLE `transactions` ADD `operation_id` text;--> statement-breakpoint
...
CREATE TABLE `__new_outbox` (
	...
	CONSTRAINT "outbox_op_check" CHECK("op" in ('insert', 'update', 'delete', 'restore', 'move'))
);
...
CREATE INDEX `transactions_operation_idx` ON `transactions` (`operation_id`);