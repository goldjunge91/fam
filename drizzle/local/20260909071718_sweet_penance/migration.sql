...
CREATE TABLE `households` (
	...
	`deleted_at` integer,
	`_dirty` integer DEFAULT false NOT NULL,
	`plus_active` integer DEFAULT false NOT NULL,
...
CREATE TABLE `fridge_items` (
	...
	`deleted_at` integer,
	`_dirty` integer DEFAULT false NOT NULL,
	CONSTRAINT "fridge_items_name_check" CHECK(length(trim("name")) between 1 and 200),
	CONSTRAINT "fridge_items_unit_check" CHECK(length(trim("unit")) > 0),
	CONSTRAINT "fridge_items_quantity_check" CHECK("quantity" >= 0
        and "quantity" <= 9999999.9
        and "quantity" * 10 = cast("quantity" * 10 as integer)),
	CONSTRAINT "fridge_items_quantity_lifecycle_check" CHECK(("deleted_at" is null and "quantity" > 0)
        or ("deleted_at" is not null and "quantity" = 0)),
	CONSTRAINT "fridge_items_package_size_check" CHECK("package_size" is null or (
        "package_size" > 0
        and "package_size" <= 9999999.9
        and "package_size" * 10 = cast("package_size" * 10 as integer)
      )),
	CONSTRAINT "fridge_items_package_size_unit_check" CHECK(("package_size" is null and "package_size_unit" is null)
        or ("package_size" is not null
          and "package_size_unit" is not null
          and length(trim("package_size_unit")) > 0))
);
--> statement-breakpoint
CREATE TABLE `storage_locations` (
	...
	`deleted_at` integer,
	`_dirty` integer DEFAULT false NOT NULL,
	CONSTRAINT "storage_locations_name_check" CHECK(length(trim("name")) between 1 and 60),
	CONSTRAINT "storage_locations_kind_check" CHECK("kind" in ('fridge', 'freezer', 'pantry', 'custom'))
);
--> statement-breakpoint
CREATE TABLE `transactions` (
	...
	`deleted_at` integer,
	`_dirty` integer DEFAULT false NOT NULL,
	CONSTRAINT "transactions_type_check" CHECK("type" in ('in', 'out', 'waste')),
	CONSTRAINT "transactions_unit_check" CHECK(length(trim("unit")) > 0),
	...
	CONSTRAINT "transactions_quantity_check" CHECK("quantity" > 0
        and "quantity" <= 9999999.9
        and "quantity" * 10 = cast("quantity" * 10 as integer)),
	CONSTRAINT "transactions_reason_check" CHECK(("type" <> 'waste' and "reason" is null)
        or ("type" = 'waste' and "reason" is not null
          and "reason" in ('expired', 'spoiled', 'other')))
);
--> statement-breakpoint
...
CREATE INDEX `fridge_items_location_idx` ON `fridge_items` (`location_id`,`deleted_at`);--> statement-breakpoint
CREATE INDEX `fridge_items_dirty_idx` ON `fridge_items` (`_dirty`) WHERE "fridge_items"."_dirty" = 1;--> statement-breakpoint
CREATE INDEX `storage_locations_hh_idx` ON `storage_locations` (`household_id`,`deleted_at`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `transactions_operation_type_idx` ON `transactions` (`operation_id`,`type`);--> statement-breakpoint
CREATE UNIQUE INDEX `transactions_reversal_idx` ON `transactions` (`reversal_of`) WHERE "transactions"."reversal_of" is not null;--> statement-breakpoint
CREATE INDEX `transactions_hh_idx` ON `transactions` (`household_id`,`updated_at`,`id`);--> statement-breakpoint
CREATE INDEX `transactions_item_idx` ON `transactions` (`fridge_item_id`,`updated_at`);--> statement-breakpoint
CREATE INDEX `transactions_dirty_idx` ON `transactions` (`_dirty`) WHERE "transactions"."_dirty" = 1;--> statement-breakpoint
...
CREATE INDEX `local_recipe_preferences_user_idx` ON `local_recipe_preferences` (`user_id`,`updated_at`);--> statement-breakpoint
...
CREATE INDEX `stores_dirty_idx` ON `stores` (`_dirty`) WHERE "stores"."_dirty" = 1;--> statement-breakpoint
...
CREATE UNIQUE INDEX `injection_plans_user_id_idx` ON `injection_plans` (`user_id`);--> statement-breakpoint
