...
function wrapperScript(ccacheBinary, ccacheDir, compiler) {
  return `#!/bin/sh
# Generated by withIosCcacheDir. Resolve project paths from this wrapper so
# the compiler also works after EAS copies the generated native project.
export CCACHE_DIR=${shellQuote(ccacheDir)}
CCACHE_BASEDIR="$(CDPATH= cd "$(dirname "$0")/.." && pwd)" || exit 1
export CCACHE_BASEDIR
export CCACHE_CONFIGPATH="$CCACHE_BASEDIR/node_modules/react-native/scripts/xcode/ccache.conf"
exec ${shellQuote(ccacheBinary)} ${compiler} "$@"
`;
}
  ...
  config = withDangerousMod(config, [
    ...
    (config) => {
      ...
      for (const [file, compiler] of [
        ...
        writeFileSync(target, wrapperScript(ccacheBinary, ccacheDir, compiler));
