...
| `08-fields-and-selection.md` | Beschreibt `Field` und Fokus, aber nicht das produktive `TextField`, Fehler + Fokus, Refs, explizite Accessibility-Props oder konkrete Auswahlrollen. Die Kontur wechselt von 1,5 auf 2 Punkte ohne Regel zur Geometriestabilität. | `Field` in `ui.tsx` zu `TextField` umbenennen und die produktiven Funktionen dort zusammenführen. Die Fokusmaße dürfen bleiben, sofern außen kein Layoutsprung entsteht; andernfalls die gewählte technische Lösung ausdrücklich in den Vertrag übernehmen. Fehler-/Auswahlmatrix und Submit-Ausnahmen ergänzen. Bestehende Verbraucher behalten den Namen und ändern nur den Importpfad. |
