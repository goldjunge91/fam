...
REVOKE ALL ON FUNCTION "public"."correct_fridge_item_quantity"(uuid, uuid, uuid, uuid, numeric, numeric, timestamp WITH time zone) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION "public"."correct_fridge_item_quantity"(uuid, uuid, uuid, uuid, numeric, numeric, timestamp WITH time zone) TO "authenticated", "postgres", "service_role";

REVOKE ALL ON FUNCTION "public"."reverse_inventory_quantity_transaction"(uuid, uuid, uuid, uuid, timestamp WITH time zone, text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION "public"."reverse_inventory_quantity_transaction"(uuid, uuid, uuid, uuid, timestamp WITH time zone, text) TO "authenticated", "postgres", "service_role";
