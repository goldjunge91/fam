...
select ok(
  has_function_privilege(
    'authenticated',
    'public.correct_fridge_item_quantity(uuid, uuid, uuid, uuid, numeric, numeric, timestamptz)',
    'execute'
  ),
...
values (
  ...
  'Korrektur-Milch', 5, 'piece', '11111111-1111-1111-1111-111111111111'
);
...
select public.correct_fridge_item_quantity(
  ...
  :'household_id', 5, 3, '2026-09-07T10:00:00Z'
...
select set_eq(
  ...
  $$ values ('out', 2::numeric, '[Manual correction]') $$,
...
select throws_ok(
  format(
    $$ select public.correct_fridge_item_quantity(
      ...
      'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      %L, 3, 2.9999, '2026-09-07T10:02:00Z'
    ) $$,
    :'household_id'
  ),
...
select is(
  ...
  0,
  'eine unpraezise Korrektur schreibt kein Ledger'
);
