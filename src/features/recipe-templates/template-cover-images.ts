 ...
 * (`assets/rezepte/*.jpg`), referenziert ueber `recipe_templates.cover_image_path`
 * (Slug, kein Supabase-Storage-Pfad — siehe Kommentar in
 * `supabase/schemas/15_recipe_templates.sql`). `require()` mit statischem
 * Pfad ist zwingend (der Metro-Bundler kann keinen dynamischen Pfad aufloesen),
 * deshalb eine explizite Map statt eines Musters wie bei User-Rezeptbildern.
 */
...
/** `null` faellt zurueck auf den Farbverlauf-Platzhalter in `RecipeArtwork`. */
