...
export async function applyInventoryConsumptionPlan(
  ...
  for (const item of plan) {
    ...
    const existing = await db.getFirstAsync<{
      ...
      'select quantity, unit, household_id from fridge_items where id = ? and deleted_at is null',
      [item.id],
    );

    if (existing === null) throw new Error('inventory_item_missing');
    if (existing.household_id !== item.household_id)
      throw new Error('inventory_household_mismatch');
    if (!Number.isFinite(existing.quantity) || existing.quantity < 0) {
      throw new Error('inventory_quantity_invalid');
    }

    // Compare under the same exclusive transaction that writes the mirror and
    // outbox. A stale plan must roll back the whole batch, not overwrite stock.
    const assertUnchanged = async (txn: SqlDatabase): Promise<void> => {
      const current = await txn.getFirstAsync<typeof existing>(
        'select quantity, unit, household_id from fridge_items where id = ? and deleted_at is null',
        [item.id],
      );
      if (current === null) throw new Error('inventory_item_missing');
      if (current.household_id !== item.household_id)
        throw new Error('inventory_household_mismatch');
      if (current.quantity !== existing.quantity || current.unit !== existing.unit) {
        throw new Error('inventory_quantity_changed');
      }
    };
    ...
    if (boundedQuantity === 0) {
      mutations.push({
        ...
        applyLocally: async (txn) => {
          await assertUnchanged(txn);
          await applyLocalMirrorWrite(txn, 'fridge_items', 'delete', { id: item.id }, nowMs);
        },
      });
    } else {
      mutations.push({
