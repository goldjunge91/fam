...
import type { LocalFridgeItem } from '@/features/fridge/use-fridge-items';
...
// Formatiert einen Naehrwert fuer die Anzeige — `null`/`undefined` (Feld
// existiert (noch) nicht im Schema oder Produkt hat keinen Bezug) wird als
// "–" dargestellt statt eine erfundene Zahl zu zeigen.
function fmtNutrient(value: number | string | null | undefined, digits: number): string {
  if (value === null || value === undefined) return '–';
  const n = Number(value);
  if (Number.isNaN(n)) return '–';
  return digits === 0 ? String(Math.round(n)) : n.toFixed(digits).replace(/\.0$/, '');
}

// Balkenbreite relativ zu einer Max-Skala von ~30g — fehlende Werte ergeben
// eine leere Leiste statt der bisherigen Mindestbreite.
function barWidth(value: number | string | null | undefined, minWidth = 8, maxScale = 30): number {
  if (value === null || value === undefined) return 0;
  const n = Number(value);
  if (Number.isNaN(n) || n <= 0) return 0;
  return Math.min(100, Math.max(minWidth, (n / maxScale) * 100));
}
...
export function ProductDetailModal({ visible, item, onClose }: ProductDetailModalProps) {
  ...
  // Kategorie, Icon, Nutri-Score, Zutaten und Allergene stehen im Schema
  // (noch) nicht zur Verfuegung — die Karten bleiben trotzdem sichtbar, nur
  // mit neutralem Leerzustand statt entfernt zu werden.
  const category = 'Lebensmittel';
  const icon = '🛒';
  const nutriScore: (typeof NUTRI_SCORES)[number]['score'] | null = null;
  const ingredientsText = 'Keine Angaben';
  ...
  const pWidth = barWidth(product?.protein_g_per_100);
  const cWidth = barWidth(product?.carbs_g_per_100);
  const sugWidth = barWidth(product?.sugar_g_per_100);
  const fWidth = barWidth(product?.fat_g_per_100);
  const satWidth = 0; // gesaettigtes Fett wird nicht persistiert
  const sWidth = barWidth(product?.salt_g_per_100, 4);
          ...
          <ScrollView
            ...
            <View style={styles.headerCard}>
              ...
              <View style={styles.nutriBadgeTop}>
                <ThemedText style={styles.nutriBadgeTopText}>{nutriScore ?? '–'}</ThemedText>
              </View>
            </View>
            ...
            {hasProductLink ? (
              <>
                  ...
                  <View style={styles.kcalRow}>
                    <View>
                      <View style={styles.kcalTextGroup}>
                        <ThemedText style={styles.kcalNumber}>
                          {fmtNutrient(product?.kcal_per_100, 0)}
                        </ThemedText>
                  ...
                  <View style={styles.macroList}>
                    ...
                    <View style={styles.macroRow}>
                      ...
                      <ThemedText type="smallBold" style={styles.macroVal}>
                        {fmtNutrient(product?.protein_g_per_100, 1)}g
                      </ThemedText>
                    </View>
                    ...
                    <View style={styles.macroRow}>
                      ...
                      <ThemedText type="smallBold" style={styles.macroVal}>
                        {fmtNutrient(product?.carbs_g_per_100, 1)}g
                      </ThemedText>
                    </View>
                    ...
                    <View style={styles.macroRowSub}>
                      ...
                      <ThemedText type="smallBold" style={styles.macroVal}>
                        {fmtNutrient(product?.sugar_g_per_100, 1)}g
                      </ThemedText>
                    </View>
                    ...
                    <View style={styles.macroRow}>
                      ...
                      <ThemedText type="smallBold" style={styles.macroVal}>
                        {fmtNutrient(product?.fat_g_per_100, 1)}g
                      </ThemedText>
                    </View>

                    {/* davon gesättigt — nicht persistiert */}
                    <View style={styles.macroRowSub}>
                      <ThemedText type="small" style={styles.macroLabelSub}>
                        davon gesättigt
                      </ThemedText>
                    ...
                    <View style={styles.macroRow}>
                      ...
                      <ThemedText type="smallBold" style={styles.macroVal}>
                        {fmtNutrient(product?.salt_g_per_100, 1)}g
                      </ThemedText>
                    </View>
                  </View>
                </View>
                ...
                <View style={styles.card}>
                  ...
                  <View style={styles.nutriRow}>
                    {NUTRI_SCORES.map((ns) => {
                      const isActive: boolean = nutriScore === ns.score;
                      return (
                        <View
                          key={ns.score}
                          style={[
                ...
                <View style={styles.card}>
                  ...
                  <ThemedText style={styles.ingredientsText}>{ingredientsText}</ThemedText>
                </View>
              ...
              <View style={styles.card}>
                <ThemedText type="small" themeColor="textSecondary">
                  Keine Nährwertdaten verfügbar für diesen Artikel.
                </ThemedText>
              </View>
            )}
