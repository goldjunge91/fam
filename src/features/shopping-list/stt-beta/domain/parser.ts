...
const DECIMAL_PERIOD_PLACEHOLDER = '\u0003';
const FIRST_QUANTITY_START =
  /(?:\d+(?:[.,]\d+)?(?:\s*[x×])?|ein(?:e|en)?|zwei|drei|vier|fünf|sechs|sieben|acht|neun|zehn|elf|zwölf|zweihundert|fünfhundert|halbes?)\s+/iu;
...
const QUANTITY_BOUNDARY =
  /(?<!ein)(?<=[\p{L}\p{N}])\s+(?=(?:\d+(?:[.,]\d+)?(?:\s*[x×])?|(?:ein\s+)?halbes?|ein(?:e|en)?|zwei|drei|vier|fünf|sechs|sieben|acht|neun|zehn|elf|zwölf|zweihundert|fünfhundert)\s+)/giu;
...
function stripLeadingSpeechContext(input: string): string {
  ...
  if (/\bund\b/iu.test(context)) {
    return input.replace(
      /^\s*(?:bitte|ich\s+(?:brauche|möchte|moechte|will)|(?:ich\s+)?brauch(?:e)?|(?:ich\s+)?möchte|(?:ich\s+)?moechte)\s+/iu,
      '',
    );
  }
...
function splitIntoSegments(input: string): string[] {
  const protectedInput = input
    .replace(/(?<=\d),(?=\d)/gu, DECIMAL_COMMA_PLACEHOLDER)
    .replace(/(?<=\d)\.(?=\d)/gu, DECIMAL_PERIOD_PLACEHOLDER)
  ...
  return protectedInput
    ...
    .map((segment) =>
      segment
        ...
        .replaceAll(DECIMAL_COMMA_PLACEHOLDER, ',')
        .replaceAll(DECIMAL_PERIOD_PLACEHOLDER, '.')
        .trim(),
    )
    .filter(Boolean);
}
...
function parseQuantity(input: string): ParsedQuantity {
  const halfMatch = input.match(/^(?:ein\s+)?halbes?\s+(.+)$/iu);
  if (halfMatch) {
    return { quantity: 0.5, remainder: halfMatch[1] };
  }
