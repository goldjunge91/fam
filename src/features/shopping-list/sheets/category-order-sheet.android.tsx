...
debugLogEvent('shopping-list.category-order-sheet.module-loaded', { variant: 'android' });
...
export function CategoryOrderSheet({ isOpen, store, onClose }: Props) {
  ...
  useEffect(() => {
    debugLogEvent('shopping-list.category-order-sheet.visibility', {
      variant: 'android',
      isOpen,
    });
    if (isOpen) {
      setOrder(resolveOrder(store));
      debugLogEvent('shopping-list.category-order-sheet.native-action', {
        variant: 'android',
        action: 'expand',
      });
