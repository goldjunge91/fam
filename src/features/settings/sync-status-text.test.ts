...
describe('describeSyncStatus', () => {
  ...
  it('meldet den Normalfall unauffaellig', () => {
    expect(describeSyncStatus({ kind: 'hidden' }, i18n.t)).toEqual({
  ...
  it('unterscheidet offline mit und ohne ausstehende Aenderungen', () => {
    expect(describeSyncStatus({ kind: 'offline', pendingCount: 0 }, i18n.t)).toMatchObject({
      text: 'Offline (Keine Internetverbindung)',
      short: 'Offline',
      tone: 'warning',
    });

    expect(describeSyncStatus({ kind: 'offline', pendingCount: 3 }, i18n.t)).toMatchObject({
  ...
  it('hebt Fehlgeschlagenes hervor', () => {
    expect(describeSyncStatus({ kind: 'failed', failedCount: 4 }, i18n.t)).toEqual({
  ...
  it('haelt die Kurzfassung kurz genug fuer die Menuezeile', () => {
    const alle = [
      describeSyncStatus({ kind: 'hidden' }, i18n.t),
      describeSyncStatus({ kind: 'offline', pendingCount: 0 }, i18n.t),
      describeSyncStatus({ kind: 'offline', pendingCount: 12 }, i18n.t),
      describeSyncStatus({ kind: 'failed', failedCount: 12 }, i18n.t),
    ];
