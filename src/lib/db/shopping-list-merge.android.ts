...
export async function buildAddOrMergeShoppingItemMutation(
  ...
  if (existing) {
    ...
    debugLogEvent('shopping-list.add-item.merge-planned', {
      variant: 'android',
  ...
  debugLogEvent('shopping-list.add-item.insert-planned', {
    variant: 'android',
    entityId: newId,
    householdId: input.household_id,
    productId: input.product_id ?? null,
...
export async function addOrMergeShoppingItem(
  ...
  debugLogEvent('shopping-list.add-item.local-queued', {
    variant: 'android',
