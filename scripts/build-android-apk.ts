...
// Die native Android-Konfiguration liegt als eigener Wert in build.gradle.
// Synchronisieren, damit versionCode/versionName des APKs zur app.json passen.
...
if (fs.existsSync(androidBuildGradlePath)) {
  ...
  androidBuildGradle = androidBuildGradle
    .replace(/(\bversionCode\s+)\d+/, `$1${newVersionCode}`)
    .replace(/(\bversionName\s+)["'][^"']+["']/, `$1"${newAppVersion}"`);
  fs.writeFileSync(androidBuildGradlePath, androidBuildGradle, 'utf8');
  logger.ok('Native Android-Version erfolgreich synchronisiert.');
} else {
  logger.warn('android/app/build.gradle nicht gefunden — native Version unverändert.');
}
...
// ------------------------------------------------------------- 6. AndroidManifest: AdMob App-ID injizieren
...
if (admobAppId) {
  ...
  if (existingEntry.test(manifest)) {
    ...
    // Füge vor </application> ein
    manifest = manifest.replace('</application>', `    ${metaTag}\n  </application>`);
  }
  fs.writeFileSync(manifestPath, manifest, 'utf8');
  logger.ok('AdMob App-ID erfolgreich ins AndroidManifest injiziert.');
} else {
  logger.warn('Kein androidAppId in app.json gefunden — AndroidManifest unverändert.');
}

// ------------------------------------------------------------- 7. Gradle Build Execution
