...
# On hit, exits 2 and writes the cached body to stderr so Codex can deliver
# it to the agent in place of the WebFetch result. Otherwise exits 0.
#
...
# file exists at .codex/sdd-cache/.debug. Toggle with `touch` / `rm`.
dbg() {
  local dir="${SDD_CACHE_ROOT:-$PWD}/.codex/sdd-cache"
...
CACHE_DIR="${SDD_CACHE_ROOT:-$PWD}/.codex/sdd-cache"
