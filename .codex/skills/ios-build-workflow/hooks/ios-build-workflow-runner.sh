...
ROOT="${IOS_BUILD_WORKFLOW_PROJECT_ROOT:-${CLAUDE_PROJECT_DIR:-}}"
if [ -n "$ROOT" ]; then
  ROOT="$(cd "$ROOT" && pwd)"
else
...
FASTPATH="${IOS_BUILD_WORKFLOW_FASTPATH:-}"
if [ -z "$FASTPATH" ]; then
  for candidate in \
    "$SCRIPT_DIR/../scripts/native-testflight-fastpath.sh" \
    "$ROOT/.codex/skills/ios-build-workflow/scripts/native-testflight-fastpath.sh" \
    "$ROOT/scripts/native-testflight-fastpath.sh"; do
    if [ -f "$candidate" ]; then
      FASTPATH="$candidate"
      break
    fi
...
if [ -z "$FASTPATH" ]; then
  printf '[ios-build] TestFlight fastpath script was not found.\n' >&2
  exit 78
fi
